...
def list_ntc_modules(module_dir, by_category=True):
    ...
    for d in files:
        ...
        if os.path.isdir(d):
            ...
            if d.endswith('__init__.py') or module in ANSIBLE_ntc_BLACKLIST:
...
def generate_parser():
    ...
    p.add_option("-T", "--template-dir", action="store", dest="template_dir", default=TEMPLATEDIR, help="directory containing Jinja2 templates")
...
def process_module(module, options, env, template, outputname, module_map, aliases):
    ...
    if doc is None:
        if (module in ansible.utils.module_docs.BLACKLIST_MODULES or
                module in ANSIBLE_ntc_BLACKLIST):
            return "SKIPPED"
        else:
...
def process_category(category, categories, options, env, template, outputname, by_category=True):
    ...
    if by_category:
        ...
        category_file_path = os.path.join(options.output_dir, "modules_list.rst")
...
def run(by_category=True, modules_title='Modules'):
    ...
    categories = list_ntc_modules(options.module_dir, by_category=by_category)
    ...
    if by_category:
        ...
        module_list_path = os.path.join(options.output_dir, "modules_list.rst")
        module_list_file = open(module_list_path, "w")
